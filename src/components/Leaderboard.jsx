...
const Leaderboard = () => {
  ...
  // Инициализируем glow эффект
  useGlowEffect('#leaderboard .feature-card')
  
  // Повторно инициализируем glow эффект после загрузки данных
  useEffect(() => {
    if (!loading) {
      // Небольшая задержка для того, чтобы DOM обновился
      const timer = setTimeout(() => {
        // Переинициализируем эффект для всех карточек на странице лидерборда
        const cards = document.querySelectorAll('#leaderboard .feature-card')
        
        // Utility functions (из useGlowEffect)
